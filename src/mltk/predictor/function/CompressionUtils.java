...
/**
 * Class for utility functions for compressing ensembles of univariate/bivariate
 * functions.
 * 
 * @author Yin Lou
 * 
...
public class CompressionUtils {
	 ...
	 * 
	 * @param attIndex
	 *            the attribute index of this regressor.
	 * @param baggedEnsemble
	 *            the bagged ensemble.
	 * @return a single compressed 1D function.
	 */
	public static Function1D compress(int attIndex,
			BaggedEnsemble baggedEnsemble) {
	 ...
	 * 
	 * @param attIndex
	 *            the attribute of this regressor.
	 * @param boostedEnsemble
	 *            the boosted ensemble.
	 * @return a single compressed 1D function.
	 */
	public static Function1D compress(int attIndex,
			BoostedEnsemble boostedEnsemble) {
		Function1D function = Function1D.getConstantFunction(attIndex, 0);
	 ...
	 */
	public static Function2D compress(int attIndex1, int attIndex2,
			BaggedEnsemble baggedEnsemble) {
		Function2D function = Function2D.getConstantFunction(attIndex1,
				attIndex2, 0);
		for (int i = 0; i < baggedEnsemble.size(); i++) {
			Predictor predictor = baggedEnsemble.get(i);
	...
	public static Function2D compress(int attIndex1, int attIndex2,
		...
		for (int i = 0; i < boostedEnsemble.size(); i++) {
			...
			if (predictor instanceof Function2D) {
				...
				func = compress(attIndex1, attIndex2,
						(BaggedEnsemble) predictor);
			} else {
				throw new IllegalArgumentException();
			}
			function.add(func);
	 ...
	 * 
	 * @param n1
	 *            the number of bins for 1st attribute.
	 * @param n2
	 *            the number of bins for 2nd attribute.
	 * @param function
	 *            the 2D function.
