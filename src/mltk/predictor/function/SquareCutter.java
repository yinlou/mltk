...
public class SquareCutter extends Learner {
	...
	public Function2D build(Instances instances) {
		...
		if (cutOnAttr2) {
			...
			if (lineSearch) {
				lineSearch(instances, f2.getIndex(), f1.getIndex(), 
						bestV2, bestV1s[0], bestV1s[1], predInt2);
			}
			...
			if (lineSearch) {
				lineSearch(instances, f1.getIndex(), f2.getIndex(),
						bestV1, bestV2s[0], bestV2s[1], predInt1);
			}
	...
	protected static void computeTable(Histogram2D hist2d, 
			CHistogram cHist1, CHistogram cHist2, Table table) {
		double sum = 0;
		double count = 0;
