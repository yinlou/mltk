...
public class RidgeLearner extends Learner {
	...
	/**
	 * Builds an L2-regularized binary classifier. Each row in the input
	...
	/**
	 * Builds an L2-regularized binary classifier on sparse inputs.
	...
	public GLM buildBinaryClassifier(int[] attrs, int[][] indices, double[][] values,
			int[] y, int maxNumIters, double lambda) {
		double[] w = new double[attrs.length];
		double intercept = 0;
	 ...
	 * feature (instead of a data point), i.e., in column-oriented format.
	...
	/**
	 * Builds an L2-regularized regressor. Each row in the input matrix
	...
	/**
	 * Builds an L2-regularized regressor on sparse inputs. Each row of
	...
	public GLM[] buildRegressors(int[] attrs, double[][] x, double[] y,
		...
		for (int g = 0; g < glms.length; g++) {
			...
			double currLoss = GLMOptimUtils.computeRidgeLoss(residualTrain, w, lambda);
			System.out.println("Model " + g + ": " + lambda + " " + currLoss + " " + StatUtils.rms(residualTrain));
	...
	public GLM[] buildRegressors(int[] attrs, int[][] indices, double[][] values,
			double[] y, int maxNumIters, double[] lambdas) {
		double[] w = new double[attrs.length];
		double intercept = 0;
	...
	protected void doOnePass(double[][] x, double[] theta, int[] y, final double tl2,
	...
	protected void doOnePass(int[][] indices, double[][] values, double[] theta,
	    int[] y, final double tl2, double[] w, double[] pTrain, double[] rTrain) {
		for (int j = 0; j < indices.length; j++) {
		  if (Math.abs(theta[j]) <= MathUtils.EPSILON) {
        continue;
      }
