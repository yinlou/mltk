...
public class FAST {
		...
		for (int i = 0; i < cHist.length; i++) {
			if (used[i]) {
				switch (attributes.get(i).getType()) {
					case BINNED:
						BinnedAttribute binnedAtt = 
							(BinnedAttribute) attributes.get(i);
						cHist[i] = new CHistogram(binnedAtt.getNumBins());
						break;
					case NOMINAL:
						NominalAttribute nominalAtt = 
							(NominalAttribute) attributes.get(i);
	...
	protected static void computeHistogram2D(Instances instances, 
			int f1, int f2, Histogram2D hist2d) {
		for (Instance instance : instances) {
	...
	protected static double getRSS(Table table, int v1, int v2, 
			double ySq, double[] pred) {
		double[] count = table.count[v1][v2];
		double[] resp = table.resp[v1][v2];
		double rss = ySq;
		double t = 0;
